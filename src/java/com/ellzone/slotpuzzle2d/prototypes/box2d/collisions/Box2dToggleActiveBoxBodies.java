...
public class Box2dToggleActiveBoxBodies extends SPPrototype implements InputProcessor {
    ...
    private void initialiseReelSlots() {
        ...
        for (int i = 0; i < 20; i++) {
            AnimatedReel animatedReel = new AnimatedReel(slotReelScrollTexture, 0, 0, spriteWidth, spriteHeight, spriteWidth, spriteHeight, 0, tweenManager);
    ...
    private void createBox(PolygonShape boxPoly) {
        BodyDef boxBodyDef = new BodyDef();
        boxBodyDef.type = BodyDef.BodyType.DynamicBody;
        boxBodyDef.position.x = -24 + (float) (Math.random() * 48);
        boxBodyDef.position.y = 10 + (float) (Math.random() * 50);
