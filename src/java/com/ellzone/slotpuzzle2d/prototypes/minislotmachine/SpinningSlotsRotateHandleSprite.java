...
public class SpinningSlotsRotateHandleSprite extends SPPrototypeTemplate {
	...
	private enum SlotHandleState { STOPPED, PULLING_DOWN, PULLING_UP };
	private SlotHandleState slotHandleState;
	private float slotHandleAngle = 0.0f;
	...
	protected void updateOverride(float dt) {
		...
		if (slotHandleState == SlotHandleState.PULLING_DOWN) {
            slotHandleAngle -= 1.5f;
			if (slotHandleAngle < -60.0f) {
				slotHandleState = SlotHandleState.PULLING_UP;
			}
        } else if (slotHandleState == SlotHandleState.PULLING_UP) {
			slotHandleAngle += 1.5f; 
			if (slotHandleAngle > -0.0f) {
				slotHandleState = SlotHandleState.STOPPED; 
			}
		}
		slotHandle.setRotation(slotHandleAngle);
	}
    ...
    private void initialiseReelSlots() {
        ...
        for (int i=0; i<3; i++) {
            AnimatedReel reel = new AnimatedReel(slotReelScrollTexture, 0, 0, spriteWidth, spriteHeight, spriteWidth, spriteHeight, 0, null, reelStoppingSound, tweenManager);
    ...
    public void handleInput(float delta) {
        if (Gdx.input.justTouched()) {
            ...
            if (slotHandle.getBoundingRectangle().contains(touch)) {
                ...
                if (reelsNotSpinning) {
					slotHandleState = SlotHandleState.PULLING_DOWN;
                    pullLeverSound.play();
                    for (AnimatedReel animatedReel : reels) {
                        animatedReel.setEndReel(random.nextInt(sprites.length - 1));
                        animatedReel.reinitialise();
